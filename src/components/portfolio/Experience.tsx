        ...
        {EDUCATION.map((edu, i) => (
          <div key={i} className="flex items-start gap-4">
            ...
            <div>
              ...
              {edu.coursework && (
                <p className="text-muted-foreground text-sm mt-1">
                  <span className="text-muted-foreground text-sm mt-1">Graduate Level Coursework:</span> {edu.coursework.join(", ")}
                </p>
              )}
            </div>
          </div>
                ...
                <div className={`w-12 h-12 rounded-lg bg-white flex items-center justify-center flex-shrink-0 overflow-hidden border border-border ${job.company === "Boston Scientific" ? "" : ""}`}>
                  {logo ? (
                    <img 
                      src={logo} 
                      alt={job.company} 
                      className={`${job.company === "Boston Scientific" ? "w-full h-full object-cover scale-150" : "w-full h-full object-contain p-1"}`}
                    />
                  ) : (
